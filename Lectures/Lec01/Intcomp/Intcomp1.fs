...
let rec eval e (env : (string * int) list) : int =
    ...
    | CstI i            -> i
    | Var x             -> lookup env x 
    //2.1 New eval function for let. uses a fold to accumulate all let bindings.
    | Let(bindings, ebody) -> 
      let env1 =
        List.fold
          (fun acc_env (x, erhs) ->
             let xval = eval erhs acc_env in
             (x, xval) :: acc_env)
          env
          bindings
      in
...
let rec freevars e : string list =
    ...
    | CstI i -> []
    | Var x  -> [x]
    (*2.2: New freevars code for let. 
    uses union to find free variables across several statements 
    and then minus to remove bound variables(here removal of varibles in their own binding are implicit)*)
      | Let(bindings, ebody) ->
          let boundVars = bindings |> List.map fst
          let rhsFVs =
              ([], bindings)
              ||> List.fold (fun acc (_, rhs) -> union (acc, freevars rhs))
          let bodyFVs = freevars ebody |> fun fv -> minus (fv, boundVars)
          union (rhsFVs, bodyFVs)
...
//2.3 helper functions to expand let bindings, used to make a list of let's into nested lets
let rec expandLets bindings body =
    match bindings with
    | [] -> body
    | (x, erhs)::rest -> Let([ (x, erhs) ], expandLets rest body)

let rec tcomp (e : expr) (cenv : string list) : texpr =
    match e with
    | CstI i -> TCstI i
    | Var x  -> TVar (getindex cenv x)
    //2.3 new let Tcomp, uses the helper function to expand bindings and then maps each individual let to a Tlet
    | Let(bindings, ebody) ->
        match bindings with
        | [] -> tcomp ebody cenv
        | (x, erhs)::rest ->
            let expanded = expandLets rest ebody
            let cenv1 = x :: cenv
            TLet(tcomp erhs cenv, tcomp expanded cenv1)
